...
struct PlateHasherAll {
    size_t operator()(const VehiclePlate& plate) const {
        size_t hash = static_cast<size_t>(plate.GetRegion());
        hash = (hash << 10) | static_cast<size_t>(plate.GetDigits());

        const auto& letters = plate.GetLetters();
        hash = (hash << 8) | static_cast<size_t>(static_cast<unsigned char>(letters[0]));
        hash = (hash << 8) | static_cast<size_t>(static_cast<unsigned char>(letters[1]));
        hash = (hash << 8) | static_cast<size_t>(static_cast<unsigned char>(letters[2]));
