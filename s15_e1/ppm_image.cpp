...
void NegateInplace(img_lib::Image &image) {
    for (int y = 0; y < image.GetHeight(); ++y) {
        ...
        for (int x = 0; x < image.GetWidth(); ++x) {
            line[x].r = static_cast<byte>(PPM_MAX - std::to_integer<int>(line[x].r));
            line[x].g = static_cast<byte>(PPM_MAX - std::to_integer<int>(line[x].g));
            line[x].b = static_cast<byte>(PPM_MAX - std::to_integer<int>(line[x].b));
        }
    }
}
}  // namespace img_lib
